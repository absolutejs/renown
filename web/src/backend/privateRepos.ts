...
export const loadAccessiblePrivateRepos = async (token: string, allowedLogins: string[], options: PrivateRepoPageOptions = {}) => {
  ...
  if (!userResponse?.ok) return { repos: [], needsGithubAuth: true, reason: "Sign in with GitHub to load private repositories.", page, perPage, total: 0, totalPages: 0, hasMore: false, query };
  ...
  if (!login || !allowedLogins.some((allowed) => allowed.toLowerCase() === login.toLowerCase())) {
    return { repos: [], needsGithubAuth: true, reason: "This session is not backed by one of your linked GitHub accounts.", page, perPage, total: 0, totalPages: 0, hasMore: false, query };
  }
  ...
  // Authenticated GitHub search covers private repositories the linked identity can access and
  // returns total_count, which gives the UI real numbered pages instead of a guessed next button.
  const search = query ? `${query} in:name is:private` : "is:private";
  const endpoint = `${GH}/search/repositories?q=${encodeURIComponent(search)}&sort=updated&order=desc&per_page=${perPage}&page=${page}`;
  const response = await fetch(endpoint, { headers: headers(token), signal: AbortSignal.timeout(20_000) }).catch(() => null);
  if (!response?.ok) return { repos: [], needsGithubAuth: true, reason: "GitHub did not allow private repository access. Reconnect GitHub to grant it.", page, perPage, total: 0, totalPages: 0, hasMore: false, query };
  ...
  const total = Array.isArray(payload) ? repos.length : Math.max(0, Number(payload.total_count ?? rows.length));
  // GitHub Repository Search exposes at most the first 1,000 matches.
  const totalPages = Math.ceil(Math.min(total, 1_000) / perPage);
