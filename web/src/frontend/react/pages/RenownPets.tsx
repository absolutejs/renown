...
type GalleryPet = { seed: string; login: string | null; handle: string; tier: string; isAi: boolean };
...
type RenownPetsProps = { cssPath?: string; pets?: GalleryPet[]; origin?: string };

export const RenownPets = ({ cssPath, pets = [], origin = "" }: RenownPetsProps) => {
      ...
      <body>
        <main className="wrap profilePage">
          ...
          <section className="card">
            ...
            <p className="muted">The latest 1/1 pets minted across renown — one per recently-active dev. Every pet is procedurally generated from a real commit, so no two are alike. Click any to see its page and owner.</p>
            {pets.length === 0
