...
describe("authenticated private repository discovery", () => {
  ...
  test("fetches only the requested GitHub page", async () => {
    ...
    globalThis.fetch = (async (input: string | URL | Request) => {
      ...
      return new Response(JSON.stringify({ total_count: 5, items: [
    ...
    expect(result).toEqual(expect.objectContaining({ page: 2, total: 5, totalPages: 3, hasMore: true }));
    expect(calls).toHaveLength(2);
  ...
  test("uses GitHub's server search without putting the term in a public page request", async () => {
    ...
    expect(result).toEqual(expect.objectContaining({ page: 2, total: 5, totalPages: 3, hasMore: true, query: "secret" }));
